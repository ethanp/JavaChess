...
public class BoardLoc {
    ...
    public static BoardLoc at(int row, int col) {
    ...
    /** I can't think of a good way to phrase what this does, but it is clear from the code. */
    ...
    /**
     * Turns an input string of either the standard A-H coordinate system, or my original format
     * "ab", where
     *
     * `a` -- row ID `b` -- column ID
     *
     * into its associated BoardLoc object. e.g. "34" would result in `BoardLoc: row 3, col 4`
     */
     ...
     */
    public BoardLoc mirror() {
        return BoardLoc.at(7 - row, col);
    }

    /**
     * @return the same BoardLoc but flipped right-to-left
     */
    public BoardLoc opposite() {
        return BoardLoc.at(row, 7 - col);
    }

    /**
     * @return the BoardLoc that would look the same as this one but after you spin the board 180˚
     */
    public BoardLoc reverse() {
        return BoardLoc.at(7 - row, 7 - col);
    }

    public BoardLoc left() {
        return left(1);
    }

    public BoardLoc left(int n) {
        return BoardLoc.at(row, col - n);
    }

    public BoardLoc right() {
        return right(1);
    }

    public BoardLoc right(int n) {
        return BoardLoc.at(row, col + n);
    }

    public Team getTerritory() {
        return row < 4 ? Team.BLACK : Team.WHITE;
    }
